  ...
  async function GetAdmin(guildId: string) {
    ...
    if (data) {
      ...
      const guildDetail: guild & { managerNames: string[] } = {
        ...
        owner: { username: 'test', nickname: data.leaderName, user_title: 'title', img_src: '', memberId: '' },
        created_at: new Date(data.createdAt),
        myRole: data.myRole,
    ...
    if (data && data.length > 0) {
      const guildList: guild[] = data.map((item) => {
        ...
        return {
          ...
          owner: { username: 'test', nickname: 'test', user_title: 'title', img_src: 'test', memberId: '' },
          created_at: new Date(1),
          myRole: 'test',
          play_style: tags.play_style,
          skill_level: tags.skill_level,
